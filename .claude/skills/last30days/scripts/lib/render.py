...
def render_compact(report: schema.Report, limit: int = 15, missing_keys: str = "none") -> str:
    ...
    if report.mode == "web-only":
        ...
        lines.append("- `BRAVE_API_KEY` → Reddit threads with real upvotes & comments")
        lines.append("- `XAI_API_KEY` → X posts with real likes & reposts")
        lines.append("- Edit your project `.env` file")
        lines.append("---")
        lines.append("")
    ...
    if report.mode == "reddit-only" and missing_keys == "x":
        ...
        lines.append("*💡 Tip: Add BRAVE_API_KEY for Reddit data and better triangulation.*")
        lines.append("")
...
def render_full_report(report: schema.Report) -> str:
    ...
    lines.append("")
    lines.append("- **Reddit:** Brave Search API")
    if report.xai_model_used:
...
def write_outputs(
    ...
    Args:
        report: Report data
        raw_brave: Raw Brave Search API response
    ...
    # Raw responses
    if raw_brave:
        with open(OUTPUT_DIR / "raw_brave.json", 'w', encoding='utf-8') as f:
            json.dump(raw_brave, f, indent=2, ensure_ascii=False)
