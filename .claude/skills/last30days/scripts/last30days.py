...
# Force UTF-8 output on Windows to handle emoji and unicode characters
...
def _search_reddit(
    ...
    """Search Reddit via Brave Search API (runs in thread).

    Returns:
        Tuple of (reddit_items, raw_brave, error)
    """
    raw_brave = None
    reddit_error = None

    if mock:
        raw_brave = load_fixture("openai_sample.json")
    else:
        try:
            raw_brave = brave_reddit.search_reddit(
    ...
    # Parse response
    reddit_items = brave_reddit.parse_reddit_response(raw_brave or {})

    # Quick retry with simpler query if few results
    if len(reddit_items) < 5 and not mock and not reddit_error:
        core = brave_reddit._extract_core_subject(topic)
        if core.lower() != topic.lower():
            try:
                retry_raw = brave_reddit.search_reddit(
                    config["BRAVE_API_KEY"],
                    core,
                    from_date, to_date,
                    depth=depth,
                )
                retry_items = brave_reddit.parse_reddit_response(retry_raw)
    ...
    return reddit_items, raw_brave, reddit_error
...
def run_research(
    ...
    Returns:
        Tuple of (reddit_items, x_items, web_needed, raw_brave, raw_xai, raw_reddit_enriched, reddit_error, x_error)

    Reddit search uses Brave Search API. X search uses xAI API.
    ...
    if sources == "web":
        ...
        return reddit_items, x_items, True, raw_brave, raw_xai, raw_reddit_enriched, reddit_error, x_error
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        ...
        if reddit_future:
            try:
                reddit_items, raw_brave, reddit_error = reddit_future.result()
    ...
    return reddit_items, x_items, web_needed, raw_brave, raw_xai, raw_reddit_enriched, reddit_error, x_error
...
def main():
    ...
    # Run research
    reddit_items, x_items, web_needed, raw_brave, raw_xai, raw_reddit_enriched, reddit_error, x_error = run_research(
        args.topic,
        sources,
        config,
        selected_models,
    ...
    # Write outputs
    render.write_outputs(report, raw_brave, raw_xai, raw_reddit_enriched)
